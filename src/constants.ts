...
export const NUTRITION_EDUCATION_CONTENT: { [key: string]: string } = {
  ...
  '糖尿病': `糖尿病營養教育的終極目標是藉由個別化的飲食計畫，將血糖、血脂與血壓維持在正常範圍內，從而預防或延緩視網膜病變、腎病變及神經病變等併發症 。
            ...
            三少一多原則：營養衛教應強調「三少一多」原則：少油、少鹽、少糖、多纖維 。少油飲食不僅有助於體重管理，更能降低低密度脂蛋白（LDL）與三酸甘油酯，減少心血管疾病風險。
            1. 定時定量攝取含醣食物（全榖雜糧、水果、乳品）。
            \n2. 避免高精製糖食物、含糖飲料及甜點。
            \n3. 增加膳食纖維攝取，多選用全榖未精製穀類及新鮮蔬菜。
            \n4. 控制油脂攝取量，少吃油炸、油煎及高飽和脂肪食物。
            \n5. 規律運動並配合醫囑按時服藥與監測血糖。`,
  '腎臟病3a-3b': `對於尚未進入透析的 CKD 1-5 期患者，教育核心在於「低蛋白飲食」與「電解質平衡」，旨在減輕腎臟過濾負擔，減少尿毒素產生 。
                 蛋白質攝取的階段性限制：在 CKD 初期（第 1-3 期），應避免高蛋白餐食，Protein 0.8-1.0g/kg 。進入第 3b 至 5 期後，必須更嚴格 Protein 0.6-0.8 g/kg ，且其中 50% 至 75% 應來自「高生理價值蛋白質」（如魚、肉、蛋、黃豆）。
  ...
  '末期腎臟病': `一旦患者進入血液透析（HD）或腹膜透析（PD），透析患者面臨的是蛋白質的大量流失與代謝廢物的間歇性清除。
               ...
               液體與鹽分平衡：HD 教育重點在於控制「兩次透析間的體重增加」，理想值應低於乾體重的 5% 。衛教技巧包括：使用有刻度的小量杯精確飲水；口渴時以稀釋檸檬水冰塊含服；嚴格限鈉以降低渴感。`,
