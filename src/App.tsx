...
export default function App() {
  ...
  const [isSarcopeniaExpanded, setIsSarcopeniaExpanded] = useState<boolean>(false);
            ...
            >
              ...
              <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="border-t border-slate-100 pt-5 mt-2 bg-slate-50/50 rounded-2xl p-4 border border-slate-100 space-y-4">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div className="lg:col-span-2 space-y-3 bg-white p-4 rounded-xl border border-slate-100 shadow-xs">
                        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 border-b border-dashed border-slate-100 pb-1.5">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const wDate = state.anthropometry.weightDate;
                              const wVal = state.anthropometry.weight;
                              if (!wDate || !wVal) {
                                alert('請先填寫/修改上方的 體重 及 體重測量日期');
                                return;
                              }

                              const weightHistoryVal = state.monitoring.weightHistory || [];
                              const existingIdx = weightHistoryVal.findIndex(h => h.date === wDate);
                              let newWeightHistory = [...weightHistoryVal];

                              if (existingIdx !== -1) {
                                newWeightHistory[existingIdx] = {
                                  ...newWeightHistory[existingIdx],
                                  weight: wVal
                                };
                              } else {
                              ...
                              const legacyHistoryVal = state.monitoring.history || [];
                              const existingLegacyIdx = legacyHistoryVal.findIndex(h => h.date === wDate);
                              let newLegacyHistory = [...legacyHistoryVal];

                              if (existingLegacyIdx !== -1) {
                                newLegacyHistory[existingLegacyIdx] = {
                                  ...newLegacyHistory[existingLegacyIdx],
                                  weight: wVal
                                };
                              } else {
                                newLegacyHistory.push({
                                  date: wDate,
                                  weight: wVal,
                                  ac: '', hba1c: '', egfr: '', tg: '', ldl: '', tc: '', uricAcid: '', bp: '', other: '體重端同步'
                                });
                              }
                              newLegacyHistory.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
                              ...
                              alert(`已成功將日期 ${wDate} 的體重資料 (${wVal} kg) 同步/更新至監測紀錄`);
                            }}
                            className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 hover:text-emerald-800 border border-emerald-200 text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 rounded-lg transition-colors flex items-center gap-1 cursor-pointer"
                          >
                            🔄 同步上方已修改的體重至監測紀錄
                          </button>
                        </div>
                  ...
                  <div className="md:col-span-4 border-t border-slate-100 pt-5 mt-2 bg-slate-50/20 p-4 rounded-xl border border-dashed border-slate-200">
                    <button
                      type="button"
                      onClick={() => setIsSarcopeniaExpanded(!isSarcopeniaExpanded)}
                      className="w-full flex items-center justify-between text-left focus:outline-none hover:text-indigo-600 transition-colors cursor-pointer"
                    >
                      ...
                      <div className="flex items-center gap-2">
                        {sarcopeniaAnalysis.asmi && sarcopeniaAnalysis.applicable && (
                          <span className={`text-[10px] sm:text-xs font-bold px-2 py-0.5 rounded-full border ${sarcopeniaAnalysis.isSarcopenia ? 'bg-amber-50 text-amber-800 border-amber-200' : 'bg-green-50 text-green-800 border-green-200'}`}>
                            {sarcopeniaAnalysis.isSarcopenia ? '⚠️ 肌少症高風險' : '✓ 肌肉量正常'}
                          </span>
                        )}
                        <span className="text-xs text-slate-500 font-semibold bg-white border border-slate-200 px-3 py-1 rounded-lg flex items-center gap-1 hover:bg-slate-50 shadow-2xs transition-colors">
                    ...
                    <AnimatePresence>
                      {isSarcopeniaExpanded && (
                        <motion.div
                          ...
                          className="mt-4 pt-1 space-y-4 overflow-hidden"
                        >
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <button
                      onClick={() => {
                        ...
                        // 1. Update Biochem History
                        const biochemHistoryVal = state.monitoring.biochemHistory || [];
                        const existingBiochemIdx = biochemHistoryVal.findIndex(h => h.date === biochemDate);
                        let logoBiochem = [...biochemHistoryVal];
                        
                        const updatedBiochem = {
                          ac: state.biochemistry.AC || '',
                          hba1c: state.biochemistry.HbA1c || '',
                          egfr: state.biochemistry.eGFR || '',
                          tg: state.biochemistry.TG || '',
                        ...
                        if (existingBiochemIdx !== -1) {
                          logoBiochem[existingBiochemIdx] = {
                            ...logoBiochem[existingBiochemIdx],
                            ...updatedBiochem,
                            date: biochemDate
                          };
                        ...
                        logoBiochem.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

                        // 2. Update Weight History
                        let logoWeight = [...(state.monitoring.weightHistory || [])];
                        if (state.anthropometry.weight) {
                          const existingWeightIdx = logoWeight.findIndex(h => h.date === weightDate);
                          if (existingWeightIdx !== -1) {
                            logoWeight[existingWeightIdx] = {
                              ...logoWeight[existingWeightIdx],
                          ...
                          logoWeight.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
                        }

                        // 3. Update Combined/Legacy History
                        let logoLegacy = [...(state.monitoring.history || [])];
                        const existingLegacyIdx = logoLegacy.findIndex(h => h.date === biochemDate);
                        ...
                        logoLegacy.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
                        ...
                        setState({
                          ...
                          monitoring: {
                            ...state.monitoring,
                            history: logoLegacy,
                            biochemHistory: logoBiochem,
                            weightHistory: logoWeight
                          }
                        });
                        alert('數據已同步至體重與生化監測紀錄 (若日期相同已直接覆蓋前次紀錄)');
                      }}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              alert(`已成功將報告日期 ${biochemDate} 的生化與體重數據同步/更新至監測紀錄`);
                            }}
                            className="bg-blue-50 hover:bg-blue-100 text-blue-700 hover:text-blue-800 border border-blue-200 text-[10px] sm:text-xs font-bold px-2 sm:px-3 py-1 rounded-lg transition-colors flex items-center gap-1 cursor-pointer shadow-2xs"
                          >
                            🔄 同步上方已修改的生化數據至監測紀錄
                          </button>
                        </div>
              ...
              <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-xs">
                    <button
                      type="button"
                      onClick={() => setShowDietDetails(!showDietDetails)}
                      className="w-full px-4 py-3.5 bg-slate-50 flex items-center justify-between text-slate-800 font-bold border-b border-slate-200 hover:text-blue-600 transition-colors focus:outline-none cursor-pointer"
                    >
                      <div className="flex items-center gap-2">
                        <ClipboardList className="w-4 h-4 text-slate-500" />
                        <span>餐次食物名稱細節列表 (Meal &amp; Food Log Details)</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-xs font-semibold text-slate-500 bg-slate-200/60 px-2.5 py-0.5 rounded-full">
                          共 {state.diet.logs.length} 項飲食紀錄
                        </span>
                        <span className="text-xs text-slate-500 font-semibold bg-white border border-slate-200 px-2.5 py-1 rounded-lg flex items-center gap-1 hover:bg-slate-100 transition-colors">
                          {showDietDetails ? (
                            <>收合 <ChevronUp className="w-3.5 h-3.5" /></>
                          ) : (
                            <>展開 <ChevronDown className="w-3.5 h-3.5" /></>
